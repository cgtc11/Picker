...
class PickerCanvas(QtWidgets.QLabel):
    ...
    def _update_maya_selection(self, is_shift):
        ...
        for idx in self.selected_indices:
            nodes_to_select.extend([n for n in self.registered_items[idx].names if cmds.objExists(n)])
    ...
    def load_resource(self, path):
        ...
        if ext in [".png", ".jpg", ".jpeg"]:
            ...
            if not pix.isNull():
                ...
                if self.first_load:
                    # 初回ロード時は画像サイズに合わせるが、以降は自由変形できるよう固定解除
...
if __name__ == "__main__":
    for w in QtWidgets.QApplication.topLevelWidgets():
        try:
            if " | " in w.windowTitle() or w.windowTitle() == "Maya Picker":

    ...
    try:
        ...
        for a in attrs:
            try:
                ...
                if nice == IKFK_ATTR: return a
            except: pass
            if a == IKFK_ATTR: return a
    except: pass
    return None

def toggle_ikfk(obj):
    attr = _resolve_attr(obj)
    if attr is None: return False, f"{obj}: '{IKFK_ATTR}' not found"
    full = f"{obj}.{attr}"
    try:
...
def draw_shape(painter, t, sr, color, is_selected):
    ...
    if t in ("rect", "rect_fill"):
        ...
        poly = QtGui.QPolygon(pts); (painter.setBrush(brush) if "_fill" in t else None); painter.drawPolygon(poly); painter.setBrush(QtCore.Qt.NoBrush)
...
class ClickRegion:
    ...
    @property
    def has_switch(self): return any(n.endswith(".Switch") for n in self.names)
    @property
    def switch_targets(self): return [n[:-len(".Switch")] for n in self.names if n.endswith(".Switch")]
    @property
    def select_names(self): return [n[:-len(".Switch")] if n.endswith(".Switch") else n for n in self.names]
...
class PickerCanvas(QtWidgets.QLabel):
    ...
    def mousePressEvent(self, event):
        mod = event.modifiers()
        if event.button() == QtCore.Qt.MouseButton.MiddleButton or (event.button() == QtCore.Qt.MouseButton.LeftButton and mod & QtCore.Qt.KeyboardModifier.AltModifier):
        ...
        if event.button() == QtCore.Qt.MouseButton.RightButton:
            if hit_idx != -1:
                ...
                if item.has_switch:
                    for obj in item.switch_targets: toggle_ikfk(obj)
            else:
    ...
    def mouseMoveEvent(self, event):
        if self.last_pan_pos:
            ...
            self.pan_requested.emit(delta); self.last_pan_pos = event.globalPosition().toPoint()
            return
        if event.buttons() & QtCore.Qt.MouseButton.LeftButton:
            if not self.is_dragging and (event.position().toPoint() - self.origin).manhattanLength() > 5: self.is_dragging = True
            if self.is_dragging:
    ...
    def mouseReleaseEvent(self, event):
        ...
        if event.button() == QtCore.Qt.MouseButton.LeftButton:
            ...
            if self.is_dragging:
                new_selections = {i for i, item in enumerate(self.registered_items) if self.selection_rect.intersects(QtCore.QRect(item.rect.x()*self.scale, item.rect.y()*self.scale, item.rect.width()*self.scale, item.rect.height()*self.scale))}
                self.selected_indices = (self.selected_indices | new_selections) if is_shift else new_selections
            else:
                raw_pos = QtCore.QPoint(event.position().x() / self.scale, event.position().y() / self.scale)
                hit_idx = next((i for i, r in enumerate(self.registered_items) if r.rect.contains(raw_pos)), -1)
                if hit_idx != -1:
            ...
            self._update_maya_selection(is_shift); self.is_dragging = False; self.selection_rect = QtCore.QRect(); self.update()

    def _update_maya_selection(self, is_shift):
        nodes_to_select = []
        for idx in self.selected_indices: nodes_to_select.extend([n for n in self.registered_items[idx].select_names if cmds.objExists(n)])
        if nodes_to_select: cmds.select(nodes_to_select, replace=not is_shift, add=is_shift)
...
class PickerPlayerMaya(QtWidgets.QWidget):
    def __init__(self, parent=get_maya_main_window()):
        ...
        self.setup_window_icon() # アイコン設定を呼び出し
    ...
    def handle_json_jump(self, target_path):
        if not os.path.isabs(target_path) and self.current_json_path:
            target_path = os.path.normpath(os.path.join(os.path.dirname(self.current_json_path), target_path))
